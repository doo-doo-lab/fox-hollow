...
const ED = [
  ...
  { t: '圣工匠敲击圣铁，火星在半空凝固成从未存在过的星座图，锤砧上的金属逐渐呈现的并非预想中的剑刃形态，而是一把可以开启其自身内部虚空结构的钥匙，它在锻造中解构了自己。', sb: 'D', br: 'I', uq: { b: { holyForge: 1 } }, e: { holyIron: 2 }, w: 1 },
  ...
  { t: '化神池的微光照亮的并非池壁，而是另一个倒悬池底的化神池，一只注视液面的采集者在倒影中看到了自己的背面，那背面的眼睛正望向它永远无法抵达的池水深处。', sb: 'D', br: 'M', uq: { b: { apotheosisPool: 1 } }, e: { ambrosia: 2 }, w: 1 },
  ...
  { t: '幼狐读懂了残卷上那段本无语义的墨迹，它意识到这些符号不是被写上去的，而是纸张本身就有的孔洞，阅读不过是让光线穿过这些孔洞，在它视网膜上投下早于文字存在的倒影。', sb: 'D', br: 'M', uq: { b: { mysteryHall: 2 } }, e: { gnosis: 1 }, w: 1 },
  ...
  { t: '旧墟遗民送来一卷记载其先祖迁徙路线的树皮地图，地图上标注的终点恰好是山谷的圣林中心那棵老树所在的位置；但地图制作于七百年前，而圣林的那棵老树年轮只有四百圈——除非地图标注的不是树的位置，而是一粒当时尚未落地的种子的位置。', sb: 'T', uq: { b: { embassy: 1, sacredGrove: 1 } }, e: { exotic: 1 }, w: 2 },
  ...
  { t: '净林的土壤在今夜黎明前裂开一条窄缝，缝里渗出的不是地下水，而是一层淡得几乎看不见的绿意；巡视的斥候没有惊动任何同伴，它只是在日志上画了一个圈，它知道有些治愈在发生之前就已经完成了，而裂缝不过是愈合留下的记忆。', br: 'I', uq: { b: { cleanForest: 1 }, phase: 4 }, e: { wood: 2 }, w: 2 },
  ...
  { t: '钛库的新混凝柱在脱模时暴露出一段不该存在的纹理——那纹理并非模板接缝或钢筋走向，而是一道蜿蜒盘旋的螺旋线，从柱基一直延伸到柱顶，仿佛混凝土在凝固之前曾有一个念头从底部升起并在到达顶端之前被永久地封存在柱身之内。', br: 'I', uq: { b: { titanVault: 1 }, phase: 4 }, e: { concrete: 1 }, w: 2 },
  { t: '一只钻井工在钛库混凝柱的基础坑里挖出一块碎陶片，陶片上的釉质纹路与精炼厂冷却池底部的裂纹走向完全一致；但精炼厂建于钛库之后，而陶片的烧制年代据旧墟遗民鉴定比山谷最早的建筑还要早——它意识到冷却池的裂纹并非随机形成，而是在复现一片更早的陶器上的图案。', br: 'I', uq: { b: { titanVault: 1 }, phase: 4 }, e: { concrete: 1 }, w: 2 },
  ...
  { t: '灵核炉在成核的瞬间，整个精炼厂的光线向内坍缩了一瞬——不是变暗，而是所有方向的光都朝炉心弯曲了同一个角度；那一瞬之后，新诞生的灵核安静地躺在冷却盘上，它的辉光并非向外发散，而是向内沉降，像一个正在默诵自身内部所有谜底的哑谜。', br: 'M', uq: { b: { coreForge: 1 }, phase: 4 }, e: { spiritCore: 2 }, w: 1 },
  { t: '这枚灵核在检测仪上显示出的辉芒频谱，与灵图阁里一卷密封卷轴上记载的某枚古代灵核数据完全一致；但密封卷轴被打开时，上面墨迹尚未干透，而卷轴的羊皮经多名长者鉴定来自三百年前——它意识到这枚灵核不是在诞生，而是在按照早已写下的参数重新发生。', br: 'M', uq: { b: { coreForge: 1 }, phase: 4 }, e: { spiritCore: 2 }, w: 1 },
  ...
  { t: '辉映台今夜将辉芒投射到净辉林的每一片叶子上，叶子在吸收辉芒后纷纷翻转过来，露出叶背银白色的绒毛；从高处看，整片净辉林变成了一面巨大而柔软的镜子，照出的不是天空，而是山谷地层深处结晶窟里那些正在缓慢生长的晶丝所弯成的方向。', br: 'M', uq: { b: { radiantGrove: 1 }, phase: 4 }, e: { radiance: 1 }, w: 2 },
  ...
  { t: '灵图阁的显图池在无风的午夜自行泛起涟漪，涟漪止息后池面上浮现的不是任何已知的图形，而是一段正在缓慢书写的文字；文字的笔画方式是结晶窟最深处的晶丝生长方式——每一笔都先在笔划的两端同时出现，然后向中间汇合，仿佛句子在被写下之前就已经知道了自己的终点。', br: 'M', uq: { b: { chartHall: 1 }, phase: 4 }, e: { spiritChart: 2 }, w: 1 },
...
const NARR = {
  oldRuin: [
    '城墙根下翻出的第一块陶片，上面写着一行字——今日迁户十二，自东谷来，授田各三亩。东谷。一只狐狸把陶片贴在鼻子上看了很久，然后说了一句让整支队伍安静下来的话：它们不是传说里的狐狸，它们是我们的邻居，我们的祖先去过那里，被它们记在了陶片上。那天傍晚所有人围坐在废墟中央，把捡到的陶片一块一块拼起来，像在拼一个打碎了的家谱。',
    '铁器集中在废墟西北角。最先出土的是一截铁针，锈得快断了，针孔里还留着一小段炭化的线，爪子一碰就碎。拿针的狐狸说它们不是只会砌石头，它们会缝东西。接着翻出了刀、钉、不知用途的小钩，还有一枚铜印。一只老斥候蹲在地上把碎铁片排成一排，从大到小，从粗到细，排完之后沉默了一会儿，说：它们有铁匠，有裁缝，有工匠，这不是一个村子，这是一座城。',
    '枯井井底堆着骨头。不是猎物的骨头——上面刻了字。和陶片的记账文字不同，骨头上的字很小很密，挤在巴掌大的一块骨片上。它们用炭粉填进凹槽拓出字来读：祭祀用羊三，鼓乐齐鸣，祈来年雨水。背面还有一行，字迹更潦草：雨来了，祭坛塌了半边，明年修。一只狐狸读完笑起来，笑到一半停住了，说：它们在记自己的日常。祭坛塌了、明年修，诸如此类。这些骨头不是档案，是它们的日记。',
    '广场东侧有一排幼崽的坟墓，排列整齐，头朝东，每座墓旁一个很小的陶罐。打开陶罐，里面是炭化的粟，颗粒分明。十七座墓，十七个陶罐。一只年轻的狐狸数完之后没说话，因为根本不用解释：粟是给幼崽路上吃的，它们相信幼崽死后会去一个很远的地方，要带干粮。这个习俗和现在一模一样。那一刻所有狐狸都明白了为什么这座废墟让它们觉得熟悉——不是建筑，是建筑里的狐狸和它们做着同样的事。它们把陶罐按原样放回去，盖好土，在上面压了一块圆石。',
    '广场正中央那棵树的下面埋着一块巨大石盘，直径大概两只狐狸伸开爪子那么宽，盘面刻着同心圆，圆心处有一个孔。这个孔的形状和之前在西北角找到的那枚铜印的柄完全吻合。铜印插进去，转动，石盘跟着转，每转一格，顺着盘边的标记望出去，对应的是一座不同的远山。这不是祭坛，这是一个方向标记台。旧墟的狐狸站在这里，转动铜印，让举杆狐狸指向要出发的方向。一只狐狸说：它们一直在指路，指给离开的人看，指给留下来的人看，指给自己看。',
    '废墟外围的弃土里筛出了二十三颗牙齿，全部来自成年狐狸。其中七颗的牙冠上有褐色环纹——釉质发育不全。老斥候说这是幼崽时期严重营养不良留下的永久印记，不是瘟疫，是饥荒——这么多狐狸小时候同时饿过肚子，说明这座城市在二十几只幼崽的童年时期发生过一次严重的粮食危机。有人活下来了，带着这些长褐环的牙齿活到成年，在废墟里工作、缝纫、刻字，最后死在这里。牙齿的主人没有离开过旧墟，一座城从头到尾都在它们的牙上。',
    ...
    '发现横梁之后的第三天，它们在城南找到了粮仓。是一个半地穴式的圆形土坑，坑壁拍得光滑坚硬，坑底还残留着一层炭化的谷物壳。规模很大，大到一只狐狸站在坑中央看不到坑沿。老斥候沿着坑边走了一圈，指给大家看坑壁上的一道裂缝：从顶部一直裂到坑底，填满了细腻的黄土。不是人为破坏，是土地本身的问题——地下水沿着裂缝渗进来，盐分上升，整个粮仓的粮食都坏了。井水咸了，粮仓坏了，它们存不住粮也靠不了这口井。这就是为什么它们决定往南走，那边有更大的河。',
    '遗物的分布顺序拼出了撤离计划的全貌。最先搬空的是粮仓，连谷壳都没剩多少，能吃的都带上了。其次是工作区，大件铁器还在，小件细巧工具全不见了——铁针、小刀、挂钩，这些最需要手工制作的东西被优先带走。最后是住房，锅碗瓢盆原样放着，不是仓皇出逃。刻名墙上最后那个刻了一半的名字停在一个不上不下的笔画上，刻刀整齐地搁在墙角——不是丢下的，是搁下的。把所有线索连起来，结论是一张时间表：工匠先走，带着工具去新的河边建房子；农夫第二批，带着种子；最后一批是老狐狸和刻名者。它们在空城里走完最后一圈，检查每一个角落，把想带走的东西带走了，把该留下的留下了，用至少三年时间分批撤离，最后一只狐狸搁下刻刀，关上门，朝南走了。',
  ...
  cloudRidge: [
    '登云岭的决定，是在旧墟横梁那封信被发现之后才正式做出的。信上说它们往南走了，但没有说它们有没有回头看，有没有在某个高处站一下，望一眼自己留下的城。一个年轻的斥候在篝火边说：如果我是它们，我会找个最高的地方，能望得最远的地方，把路标留在那里。这句话让所有狐狸同时抬头看向了西北方那道终年云雾缭绕的山脉。第二天天没亮，它们就出发了。',
    '上山的路上，它们发现了第一枚狐狸掌印。印在石阶旁边的岩壁上，五指并拢，按得很深，掌垫的纹路在石面上保留得比任何刻刀都清晰。不是意外留下的，是故意按的——按完之后有人用炭灰沿着掌印描了一圈，让它在石头上更显眼。然后每隔一段就会出现一个，掌印有大有小，按的位置有高有低。最低的那个只有拇指大，是一个幼崽的爪印，按在一个需要蹲下来才能摸到的石缝旁边。一只狐狸说：这不是标记，这是排队。走在前面的按一个，后面的跟着按一个。它们把全家福留在了石壁上，从山脚一直排到云深处。',
    '岔路口的路标是用三块石头叠起来的。最底下那块最大，深深嵌在土里，上面两块依次减小，互相咬合得不用任何泥土都不会歪倒。这不是随便堆的，是精心磨合过的，石头的接触面上都有专门凿出来的卡槽。它们把最上面那块石头翻开，在背面看到了字：左，旧墟来者。右，南河谷新地。这行字解答了一切。旧墟的狐狸在往南迁徙的途中，有一部分选择离开队伍，登上云岭——不是去看风景，是去刻路标。它们在这里设了指路点，为的是让后面来的狐狸能知道往南怎么走，也为的是让将来从南边回来的狐狸能找到回旧墟的路。它们没有忘记任何方向。',
    '峰顶平台上的那块扁平石头，就是旧墟石板广场的那一块。不是同一块石头，但磨得一样平，凿着同样的同心圆，连圆心孔的尺寸都完全吻合。它们把旧墟带回来的那枚铜印插进去，转了三个刻度，铜杆指向了正南方——南河谷的方向。然后它们发现石头的底座上另有刻字，字很小，藏在背面，对着山谷的位置：此峰可观两处。一是旧墟，二是新河。晴天可见炊烟。一只狐狸跪下来，把眼睛凑到铜杆顶端的扁圆孔上一看，扁圆孔正好框住了很远很远的地方，那里隐约能见到一条银色的细线，正是它们春天在南边找到的那条大河。这只铜杆不是标记方向的，是一个望孔——举杆的狐狸通过它看着另一座城，一座它们即将抵达的城。站在云岭上，它同时看到了自己离开的地方和将要去的地方。',
    '封在岩石侧面松脂里的那只幼崽爪印，是所有发现里最小的一个。松脂不是山上常见的松树类型产的，品种来自山下的矮松林，是被人特意带上来的，裹着还没凝固的松脂一路爬到峰顶，涂在岩壁上，然后让一只幼崽把爪印按进了松脂里。最后它把一撮自己尾巴尖的毛也封了进去。几百年过去了，松脂变成了半透明的琥珀，那只幼崽的爪印和那撮灰毛还保持着刚按进去的姿势，像是昨天才发生的事。它们没有文字记录这场攀登，但它们用松脂封存了一家三代走完这段路的身体档案。老斥候把琥珀凑近鼻子闻了闻，说：还有松油的味道，它们刚走不久。这句话让在场的所有狐狸都回头看了看来路，好像真的会在雾里看见一只灰毛狐狸领着幼崽，正从山顶往下望。',
    '在望孔的延伸线尽头，它们把沿途收集到的所有坐标信息编织进一幅完整的地图：旧墟的瓦当朝向、刻名墙的东南偏角、粮仓的冷季存粮、云岭石阶上由大至小按下的掌印、岔路口的定向石标、封在松脂里的幼崽爪印——所有这些选择，都不是偶然。旧墟的狐狸在城墙地图上标注了一条完整的、从旧墟到南河谷的迁徙路线，云岭是其中预设的第三座定向点，而在南河谷新城的遗址中，它们找到了与旧墟结构对偶的陶罐、向南继续延伸的石子路基、以及刻着"此去有水"的火成岩石碑，碑文笔迹和云岭峰顶那块"此峰可观两处"完全吻合。也就是说，它们把方向标定在了云岭的最高处，给后来者——给几百年后的我们——留了一条有据可查的路。做完这些之后，它们下山往南走了。上山时是四只狐狸，下山时还是四只，但它们觉得自己不再只是四只——它们是一条路的一部分，路的这一端连着它们的爪子，那一端连着祖先的爪子。',
    '它们在峰顶平台边缘的岩石上发现了一处被凿掉的字。凿痕很深，凿了不止一次，像是在反复确认一个字都不剩。但在侧对着晨光的一个极偏的斜角下，残留下来的最浅的一道笔划在特定角度逆光时现出一道锐利的暗痕——那不是一个字，是一道指方向的箭头，箭头朝南。凿掉它不是要隐藏它，恰恰相反。它们把字抹平，是不想让风雪磨损它，想让这个方向变成一个只有在日出时分、从特定的角度才能看见的光影标记。不是消除信息，是保存信息的最高形式——把它藏进光里。一只狐狸对着那个角度站了很久，然后把眼睛移开，说了一句话：它们连指路都不愿意直说，它们留给我们的，都是要自己去找的。',
    '它们在峰顶停留的最后一天傍晚，雾突然散了。不是缓慢消散，是整片云海在下沉的落日中骤然塌陷，露出群山之间所有隐蔽的褶皱。一个年轻的斥候抓住了这个珍贵的窗口期，借着暮色和金星的微光，在望孔正南方向极远处的山脉缺口里，辨认出了一道斜出的山脊——不像是天然台地。它立刻转动铜印，将视线从正南偏西调整了一个刻度，当缺口、山脊与举杆狐狸的扁圆孔三点一线时，它看清了那道山脊上有一块半裸露的石面，石面上隐隐有一条垂直线，不是裂缝，是一根石碑。几百年过去了，它还在那里，还站着。铜杆的扁圆孔构成了一道跨越代际的准星——举杆的狐狸穿过它看着将去的新河，后来的狐狸穿过它看见了祖先留下的碑。它们带着这个发现下山了。从今往后，狐灵祠的香火有了方向——不是对着天空祈愿，是朝着正南偏西一个刻度的那道石门，那里有祖先留给它们的路标，站在那里，可以同时望见旧墟、新河、以及我们至今还在找的第三座城。',
  ],
