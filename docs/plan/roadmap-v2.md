...
| 2.9 | 成就系统基础框架 | ✅ 已完成 | (无git仓库-待初始化后补填) | §五 2.9 |
...
| 步骤 | 标题 | 状态 | commit | 详细任务卡 |
|------|------|------|--------|---------|
| 3.1 | 工业 B 激活（oil/plate/concrete + 能量系统） | ✅ 已完成 | (无git仓库-待初始化后补填) | §六 3.1 |
| 3.2 | 灵修 B 构建（spiritInk/sigil/resonance + 灵脉消耗） | ✅ 已完成 | (无git仓库-待初始化后补填) | §六 3.2 |
| 3.3 | 神启 Phase A（宗教页签 + 虔诚/圣油 + 神恩 + 仪式） | ✅ 已完成 | 88e0a72 | §六 3.3 |
| 3.4 | 占卜子系统（每年 1 次决策） | ✅ 已完成 | (output-无git) | §六 3.4 |
...
| 步骤 | 标题 | 状态 | commit | 详细任务卡 |
|------|------|------|--------|---------|
| 4.1 | 工业 C — 精金时代（寒钛/合金 + 能量产消博弈） | ✅ 已完成 | (output-无git) | §七 4.1 |
| 4.2 | 灵修 C — 化形（晶丝/辉芒 + 灵脉消耗端） | ✅ 已完成 | (output-无git) | §七 4.2 |
| 4.3 | 神启 Phase B — 分叉落地（教团/秘仪） | ✅ 已完成 | bd3d180 | §七 4.3 |
| 4.4 | 通达 Phase B — 结邦（邦交系统 7 族 × 深度 1-2） | ✅ 已完成 | (output-无git) | §七 4.4 |
| 4.5 | 6 神选定（宗教页签新区域） | ⬜ 未开始 | - | §七 4.5 |
| 4.6 | 事件池扩充（教团/秘仪/邦交/工业C/灵修C） | ⬜ 未开始 | - | §七 4.6 |
| 4.7 | 阶段四验收清单 | ⬜ 未开始 | - | §七 4.7 |
...
> 6 神占位设计已并入 [branch-divine.md §十三](branch-divine.md)（含 6 神列表、改宗规则、数据结构草案、Phase 4 开发前待补完清单）。下方为 roadmap 侧摘要。

**6 神**（占位）：山神 / 月狐 / 篝火神 / 雾中无名 / 祖灵集合 / 第六神
...
- [ ] 工业 C：精金资源链完整可用，能量产消博弈生效
- [ ] 灵修 C：化形建筑完整可用，灵脉消耗端上线
- [ ] 教团路内容完整可用（工业+神启玩家）
- [ ] 秘仪路内容完整可用（灵修+神启玩家）
- [ ] 教令系统：发布→buff 生效→到期消失，最多 3 同时
- [ ] 飞升阶梯：第 1-2 门可开，不可逆确认，永久效果正确
- [ ] 6 神可选，改宗代价正确
- [ ] 通达 Phase B：邦交系统 7 族 ×深度 1-2 完整可用，邦书/异珍资源链正常
- [ ] `sb: 'D'` 分叉内容按主线自动过滤（工业玩家看不到秘仪内容，反之亦然）
- [ ] 四条内容线均有推进，无空转
